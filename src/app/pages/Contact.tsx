    ...
    <div>
      ...
      <section className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 pt-20 pb-12 md:pt-28 md:pb-16">
        <div className="max-w-2xl reveal">
          ...
          <h1 style={{ letterSpacing: '-0.02em' }}>Tell us about your project.</h1>
        </div>
      </section>
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 py-16 md:py-24">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
            ...
            <div className="lg:col-span-5 reveal" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <p style={s.body}>
                The best way to start is to tell us a little about what you're working on. The first conversation is free and carries no obligation, we'll give you an honest view on whether we're the right people to help.
              </p>
              <p style={s.body}>
              ...
              {/* What happens next */}
              <div style={{ marginTop: '0.75rem', paddingTop: '1.75rem', borderTop: '2px solid var(--bau-black)' }}>
                <p style={{ ...s.eyebrow, marginBottom: '1.25rem' }}>What happens next</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {[
                    ['01', 'We read every enquiry and reply within two working days.'],
                    ['02', 'If it looks like a fit, we set up a short call, free, to talk it through.'],
                    ['03', 'You get an honest view and, if you want to go ahead, a fixed fee in writing.'],
                  ].map(([n, t]) => (
                    <div key={n} style={{ display: 'flex', gap: '1rem', alignItems: 'baseline' }}>
                      <span style={{ ...s.eyebrow, opacity: 0.35, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{n}</span>
                      <span style={{ ...s.body, fontSize: '0.875rem' }}>{t}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginTop: '0.5rem' }}>
                ...
                <a href="mailto:design@noorast.co.uk"
                  style={{ fontSize: '1rem', color: 'var(--foreground)', textDecoration: 'none', borderBottom: '1.5px solid var(--ink)', paddingBottom: 2 }}>
                  design@noorast.co.uk
                </a>
                {/* TODO(Aun): when a booking calendar exists (Cal.com / Calendly), add a
                    "Book a call" link here pointing to it. Do not embed a fake widget. */}
              </div>
            ...
            <div className="lg:col-span-6 lg:col-start-7 reveal" style={{ transitionDelay: '0.1s' }}>
              {sent ? (
                <div style={{ padding: '2.5rem', background: '#ffffff', border: '2px solid var(--bau-black)' }}>
                  <p style={{ fontSize: '1.25rem', fontWeight: 500, color: 'var(--foreground)', marginBottom: '0.75rem', letterSpacing: '-0.01em' }}>Received, thank you.</p>
                ...
                <form name="enquiry" method="POST" data-netlify="true" netlify-honeypot="bot-field" onSubmit={handleSubmit}
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '1.75rem', marginTop: '0.5rem' }}>
                    <button type="submit" disabled={sending}
                      style={{ background: 'var(--foreground)', color: 'var(--background)', border: 'none', cursor: sending ? 'wait' : 'pointer', padding: '0.9375rem 2.5rem', fontSize: '0.75rem', letterSpacing: '0.06em', textTransform: 'uppercase', opacity: sending ? 0.6 : 1, transition: 'opacity 0.2s' }}>
      ...
      <section className="joint">
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 py-16 flex flex-wrap items-center justify-between gap-8 reveal">
          ...
          <div style={{ display: 'flex', gap: '2.5rem', flexWrap: 'wrap' }}>
            ...
            <Link to="/fee-guide" style={{ ...s.link, color: 'var(--muted-foreground)', borderBottom: '2px solid var(--bau-black)' }}>Fee guide →</Link>
          </div>
        </div>
      </section>
    </div>
